...
def _http_get_json(url: str, *, ttl_seconds: int, retries: int = 3, cache_prefix: str = "http"):
    key = cache_key(cache_prefix, url=url)
    ...
    raise RuntimeError(f"Request failed for URL: {url}") from last_error


def _extract_hourly_values(js, iso_date):
    hourly = js.get("hourly", {}) or {}
    daily = js.get("daily", {}) or {}

    times = hourly.get("time", []) or []
    temps = hourly.get("temperature_2m", []) or []
    rhs = hourly.get("relative_humidity_2m", hourly.get("relativehumidity_2m", [])) or []
    winds = hourly.get("wind_speed_10m", hourly.get("windspeed_10m", [])) or []
    wind_dirs = hourly.get("wind_direction_10m", hourly.get("winddirection_10m", [])) or [None] * len(times)
    ...
    try:
        ...
        if idxs:
            ...
            dir_vals = [wind_dirs[i] for i in idxs if i < len(wind_dirs) and wind_dirs[i] is not None]
    ...
    precip_list = daily.get("precipitation_sum", [0]) or [0]
    precip = precip_list[0] if precip_list else 0

    return {
        "temp": round(_safe_float(temp), 1) if pd.notna(_safe_float(temp)) else "N/A",
        "rh": round(_safe_float(rh), 1) if pd.notna(_safe_float(rh)) else "N/A",
        "wind": round(_safe_float(wind), 1) if pd.notna(_safe_float(wind)) else "N/A",
        "precip": round(_safe_float(precip, 0.0), 1),
        "wind_dir_deg": round(_safe_float(wind_dir_deg), 1) if pd.notna(_safe_float(wind_dir_deg)) else None,
...
def get_weather_noon(lat, lon, iso_date):
    ...
    for the selected local day, using Open-Meteo hourly weather around 13:00 local time.
    """
    url = (
    ...
    try:
        js = _http_get_json(url, ttl_seconds=15 * 60, cache_prefix="openmeteo_noon")
        out = _extract_hourly_values(js, iso_date)
        out.update(
            {
                "source_status": "live",
                "source_name": "open-meteo-forecast",
                "fetched_at_utc": datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),
            }
        )
        return out
    except Exception as exc:
        print(f"[WARN] Open-Meteo weather request failed for ({lat}, {lon}) on {iso_date}: {repr(exc)}")
        return {
            "temp": "N/A",
            "rh": "N/A",
            "wind": "N/A",
...
def _build_daily_weather_df_from_json(js, start_date, end_date):
    ...
    rhs = hourly.get("relative_humidity_2m", hourly.get("relativehumidity_2m", [])) or []
    winds = hourly.get("wind_speed_10m", hourly.get("windspeed_10m", [])) or []
    wind_dirs = hourly.get("wind_direction_10m", hourly.get("winddirection_10m", [])) or []
...
def get_historical_daily_weather(lat, lon, start_date, end_date):
    """
    Historical/reanalysis daily weather series built from noon-like hourly values
    plus daily precipitation totals. Used to warm-start CFFWIS from antecedent conditions.
    """
    ...
    try:
        js = _http_get_json(url, ttl_seconds=24 * 60 * 60, cache_prefix="openmeteo_hist")
        df = _build_daily_weather_df_from_json(js, start_date, end_date)
        if not df.empty:
            return df
    except Exception as exc:
        print(f"[WARN] Open-Meteo historical weather failed for ({lat}, {lon}): {repr(exc)}")
    ...
    try:
        js = _http_get_json(fallback_url, ttl_seconds=6 * 60 * 60, cache_prefix="openmeteo_hist_fallback")
...
def compute_fwi_sequence_xclim(weather_df, lat, ffmc0=85.0, dmc0=6.0, dc0=15.0):
    """
    Compute a stateful multi-day CFFWIS sequence using daily noon-like weather.
    """
    required = ["date", "temp", "rh", "wind", "precip"]
    missing = [c for c in required if c not in weather_df.columns]
    if missing:
        raise ValueError(f"Missing required weather columns: {missing}")

    if weather_df.empty:
        out = weather_df.copy()
        for col in ["FFMC", "DMC", "DC", "ISI", "BUI", "FWI", "FWI_Risk"]:
            out[col] = []
        return out
...
def get_rolling_observed_fire_state(lat, lon, lookback_days=90, end_date=None):
    """
    Reconstruct the latest FFMC/DMC/DC state from a rolling historical weather sequence.
    """
    if end_date is None:
    ...
    if hist_df.empty:
        ...
        cache.set(key, state, ttl_seconds=6 * 60 * 60)
        return state
...
def get_fwi_xclim(
    ...
    """
    Compute FWI for a single day using xclim's Canadian Forest Fire Weather Index System (CFFWIS),
    warm-started from a rolling antecedent moisture state whenever explicit initials are not supplied.
    """
    iso = date_for if date_for else date.today().isoformat()
    key = cache_key("daily_fwi", lat=round(float(lat), 4), lon=round(float(lon), 4), date=iso)
    cached = cache.get(key)
    if cached is not None:
        return cached

    if ffmc_init is None or dmc_init is None or dc_init is None:
        prev_day = datetime.fromisoformat(iso).date() - timedelta(days=1)
    ...
    if w["temp"] == "N/A" or w["rh"] == "N/A" or w["wind"] == "N/A":
        cache.set(key, 0.0, ttl_seconds=15 * 60)
        return 0.0
    ...
    cache.set(key, fwi_value, ttl_seconds=15 * 60)
    return fwi_value
...
def get_monthly_fwi_xclim(
    ...
    """
    Approximate seasonal monthly FWI by expanding monthly climatological meteorology
    to a daily sequence and then running CFFWIS sequentially across the year.
    """
    year = _safe_power_year(year)
    key = (round(float(lat), 4), round(float(lon), 4), year)
    ...
    try:
        data_json = _http_get_json(_power_monthly_url(lat, lon, year), ttl_seconds=24 * 60 * 60, cache_prefix="power_monthly")
    ...
    try:
        ...
        if missing_months:
            data_clim = _http_get_json(_power_climatology_url(lat, lon), ttl_seconds=7 * 24 * 60 * 60, cache_prefix="power_climatology")
        ...
        rows.append(
            {
                ...
                "temp": _safe_float(tas_vals[month_idx]),
                "rh": _safe_float(hurs_vals[month_idx]),
                "wind": _safe_float(wind_vals[month_idx]) * 3.6,
                "precip": _safe_float(pr_totals[month_idx]) / days_in_month,
            }
        )

    daily_df = pd.DataFrame(rows)
    daily_seq = compute_fwi_sequence_xclim(daily_df, lat=lat, ffmc0=ffmc_init, dmc0=dmc_init, dc0=dc_init)
    monthly_fwi = (
        daily_seq.assign(Month=pd.to_datetime(daily_seq["date"]).dt.month)
        .groupby("Month")["FWI"]
        .mean()
        .reindex(range(1, 13))
        .fillna(0.0)
...
def get_openmeteo_14day_weather(lat, lon, start_date=None, horizon=14):
    """
    Fetch multi-day forecast weather from Open-Meteo using 13:00 local values
    for temp/rh/wind/direction and daily precipitation totals.
    """
    ...
    try:
        js = _http_get_json(url, ttl_seconds=60 * 60, cache_prefix="openmeteo_14day")
        return _build_daily_weather_df_from_json(js, start_date, end_date)
    except Exception as e:
...
def get_14day_fire_forecast(lat, lon, base_fsi, ffmc0=None, dmc0=None, dc0=None):
    ...
    - warm-start with observed latest FFMC/DMC/DC
    - compute stateful FWI sequence
    - apply short-term FSI adjustment
    - compute FRI
    """
    ...
    if ffmc0 is None or dmc0 is None or dc0 is None:
        observed_state = get_rolling_observed_fire_state(lat, lon, lookback_days=90, end_date=date.today())
        ffmc0 = observed_state["ffmc"]
        dmc0 = observed_state["dmc"]
        dc0 = observed_state["dc"]

    fwi_df = compute_fwi_sequence_xclim(
        weather_df, lat=lat, ffmc0=float(ffmc0), dmc0=float(dmc0), dc0=float(dc0)
    )
